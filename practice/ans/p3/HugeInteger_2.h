...
bool HugeInteger< T >::operator<( const HugeInteger< T > &right ) const
{
   ...
   if(integer.size() > right.integer.size())
      ...
      for(int i=integer.size()-1; i>=0; i--){
         auto v1 = *(integer.begin()+i);
         auto v2 = *(right.integer.begin()+i);
...
HugeInteger< T > HugeInteger< T >::operator*( HugeInteger< T > &op2 )
{
   ...
   int cnt = 0;
   typename T::iterator it1 = my.integer.begin();
   typename T::iterator it2 = other.integer.begin();
...
{
   typename U::iterator it = hugeInteger.integer.end() - 1;
