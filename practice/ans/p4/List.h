...
public:
      ...
      : myData()
   {
      ...
      if(right.size() > 0){
         ...
         while(it != right.myData.myHead){
            push_back(Ty(it->myVal));
            it = it->next;
         }
      }
   }
   ...
   void push_front( const Ty &val )
   {
      ...
      newNode->myVal = val;
      node* frontNode = myData.myHead->next;
   ...
   void pop_front()
   {
      if( myData.mySize > 0 )
      {
         if(myData.mySize == 1){
            ...
            node* frontNode = myData.myHead->next;
            frontNode->next->prev = myData.myHead;
            myData.myHead->next = frontNode->next;
            myData.mySize -= 1;
         }
   ...
   void push_back( const Ty &val )
   {
      node* newNode = new node;
      newNode->myVal = val;
      node* backNode = myData.myHead->prev;
   ...
   void pop_back()
   {
      if( myData.mySize > 0 )
      {
         if(myData.mySize == 1){
            ...
            node* backNode = myData.myHead->prev;
            backNode->prev->next = myData.myHead;
            myData.myHead->prev = backNode->prev;
            myData.mySize -= 1;
         }
